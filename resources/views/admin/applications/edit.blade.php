    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.applications.update", [$application->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('form_no') ? 'is-invalid' : '' }}" type="text" name="form_no" id="form_no" value="{{ old('form_no', $application->form_no) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id" id="client_id" required>
                    @foreach($clients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('client_id') ? old('client_id') : $application->client->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('plot') ? 'is-invalid' : '' }}" name="plot_id" id="plot_id" required>
                    @foreach($plots as $id => $entry)
                        <option value="{{ $id }}" {{ (old('plot_id') ? old('plot_id') : $application->plot->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('plot'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('dealer') ? 'is-invalid' : '' }}" name="dealer_id" id="dealer_id">
                    @foreach($dealers as $id => $entry)
                        <option value="{{ $id }}" {{ (old('dealer_id') ? old('dealer_id') : $application->dealer->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('discount') ? 'is-invalid' : '' }}" type="number" name="discount" id="discount" value="{{ old('discount', $application->discount) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('total_after_discount') ? 'is-invalid' : '' }}" type="number" name="total_after_discount" id="total_after_discount" value="{{ old('total_after_discount', $application->total_after_discount) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comments') ? 'is-invalid' : '' }}" name="comments" id="comments">{{ old('comments', $application->comments) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('payment_type') ? 'is-invalid' : '' }}" name="payment_type" id="payment_type" required>
                    ...
                    @foreach(App\Models\Application::PAYMENT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('payment_type', $application->payment_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="no_of_installments">{{ trans('cruds.application.fields.no_of_installments') }}</label>
                <input class="form-control {{ $errors->has('no_of_installments') ? 'is-invalid' : '' }}" type="number" name="no_of_installments" id="no_of_installments" value="{{ old('no_of_installments', $application->no_of_installments) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('installments_period') ? 'is-invalid' : '' }}" name="installments_period" id="installments_period">
                    ...
                    @foreach(App\Models\Application::INSTALLMENTS_PERIOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('installments_period', $application->installments_period) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
